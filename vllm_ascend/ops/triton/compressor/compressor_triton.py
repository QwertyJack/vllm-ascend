...
@triton.jit
def _proj_kernel(out_ptr, x_ptr, w_ptr, M,
                 IN_DIM: tl.constexpr, OUT_DIM: tl.constexpr,
    ...
    for tile_id in range(pid, total_tiles, num_progs):
        ...
        group_m_tiles = (m_tile_count - group_m_start
                         if (m_tile_count - group_m_start) < GROUP else GROUP)
        ...
        for k0 in range(0, IN_DIM, BK):
            ...
            x = tl.load(x_ptr + rows[:, None] * IN_DIM + offs_k[None, :],
                        mask=row_valid[:, None], other=0.0)
            w = tl.load(w_ptr + offs_k[:, None] + cols[None, :] * IN_DIM)
            acc = tl.dot(x, w, acc)
        tl.store(out_ptr + rows[:, None] * OUT_DIM + cols[None, :], acc,
                 mask=row_valid[:, None])
...
@triton.jit
def _pooled_blocked(group_idx, start_pos, seg_row_base, cache_row,
                    kv_ptr, score_ptr, cache_ptr, offs_h,
                    CACHE_SIZE: tl.constexpr, HEAD_DIM: tl.constexpr,
                    RATIO: tl.constexpr, RATIO_PAD: tl.constexpr,
                    CHUNK_ROWS: tl.constexpr, HAS_RES):
    ...
    for chunk0 in range(0, RATIO_PAD, CHUNK_ROWS):
        ...
        in_seg = seg_off >= 0
        score_seg = tl.load(score_ptr + (seg_row_base + seg_off)[:, None] * HEAD_DIM + offs_h[None, :],
                            mask=(row_valid & in_seg)[:, None], other=0.0)
        if HAS_RES:
            slot = token_pos % CACHE_SIZE
            score_cache = tl.load(cache_ptr + (cache_row * CACHE_SIZE + slot[:, None]) * 2 * HEAD_DIM + HEAD_DIM + offs_h[None, :],
                                  mask=(row_valid & (seg_off < 0))[:, None], other=0.0)
    ...
    for chunk0 in range(0, RATIO_PAD, CHUNK_ROWS):
        ...
        kv_seg = tl.load(kv_ptr + (seg_row_base + seg_off)[:, None] * HEAD_DIM + offs_h[None, :],
                         mask=(row_valid & in_seg)[:, None], other=0.0)
        if HAS_RES:
            slot = token_pos % CACHE_SIZE
            kv_cache = tl.load(cache_ptr + (cache_row * CACHE_SIZE + slot[:, None]) * 2 * HEAD_DIM + offs_h[None, :],
                               mask=(row_valid & (seg_off < 0))[:, None], other=0.0)
...
@triton.jit
def _pool_kernel(
    out_ptr, kv_ptr, score_ptr, cache_ptr, norm_w_ptr,
    ...
    RATIO: tl.constexpr, RATIO_PAD: tl.constexpr, CHUNK_ROWS: tl.constexpr,
    eps: tl.constexpr, SINGLE_BLOCK: tl.constexpr,
    NO_PAD: tl.constexpr,
):
    pid = tl.program_id(0)
    ...
    for task_id in range(pid, total_group_slots, tl.num_programs(0)):
        ...
        if local_group < groups_in_batch:
            ...
            if residual > 0:
                ...
                pooled = _pooled_blocked(group_idx, start_pos, seg_row_base, cache_row,
                                         kv_ptr, score_ptr, cache_ptr, offs_h,
                                         CACHE_SIZE, HEAD_DIM, RATIO, RATIO_PAD, CHUNK_ROWS, True)
            else:
                ...
                if SINGLE_BLOCK:
                    if NO_PAD:
                        ...
                        row_valid = rows < RATIO
                        score = tl.load(score_ptr + (seg_row_base + seg_off)[:, None] * HEAD_DIM + offs_h[None, :],
                                        mask=row_valid[:, None], other=-float("inf"))
                        score_max = tl.max(score, axis=0)
                        e = tl.exp(score - score_max[None, :])
                    ...
                    pooled = _pooled_blocked(group_idx, start_pos, seg_row_base, cache_row,
                                             kv_ptr, score_ptr, cache_ptr, offs_h,
                                             CACHE_SIZE, HEAD_DIM, RATIO, RATIO_PAD, CHUNK_ROWS, False)
...
def _cache_update_kernel(
    kv_ptr, score_ptr, cache_ptr,
    meta_ptr, block_table_ptr,
    total_write_slots,
...
class _MetaUploadRing:
    ...
    def __init__(self, n_int, dev):
        self.pinned_staging = [torch.empty(n_int, dtype=torch.int32).pin_memory()
                               for _ in range(_META_RING_SIZE)]
        self.device_slots = [torch.empty(n_int, dtype=torch.int32, device=dev)
                             for _ in range(_META_RING_SIZE)]
...
def _cube_core_num():
    ...
    if _CUBE_CORE_NUM is None:
        ...
        _CUBE_CORE_NUM = driver.active.utils.get_device_properties(
            torch.npu.current_device())["num_aicore"]
    return _CUBE_CORE_NUM


def compressor(x, wkv, wgate, state_cache, cmp_ratio, norm_w,
               state_block_table=None, cu_seqlens=None, seqused=None,
               start_pos=None, num_cores=None):
    """返回 cmp_kv（bf16）。
    ...
    assert hidden_dim == 5120 and head_dim == 512, \
        f"规格约束 hidden=5120/D=512, got {hidden_dim}/{head_dim}"
    ratio = int(cmp_ratio)
    assert 2 <= ratio <= 128
    # batch 数与每段长度（布局归一）
    ...
    dev = x.device
    # 每段有效组数与输出布局
    group_counts = [(spb + used_lens[b]) // ratio - spb // ratio
                    for b, spb in enumerate(start_pos_list)]
    ...
    if is_packed:
        ...
        out = torch.zeros(min(total_tokens, total_tokens // ratio + num_batch), head_dim,
                          dtype=torch.bfloat16, device=dev)
    else:
        out_row_offsets = [b * out_rows_per_batch for b in range(num_batch)]
        out = torch.zeros(num_batch, out_rows_per_batch, head_dim,
                          dtype=torch.bfloat16, device=dev)
    ...
    block_table_ptr = (state_block_table.to(torch.int32).to(dev) if state_block_table is not None
    ...
    gemm_grid = lambda META: (min(cores, triton.cdiv(total_tokens, META['BM'])
                                  * (head_dim // META['BN'])),)
    _proj_kernel[gemm_grid](kv, x_2d, wkv, total_tokens,
                            IN_DIM=hidden_dim, OUT_DIM=head_dim)
    _proj_kernel[gemm_grid](score, x_2d, wgate, total_tokens,
                            IN_DIM=hidden_dim, OUT_DIM=head_dim)
    ...
    if total_group_slots > 0:
        ...
        _pool_kernel[pool_grid](
            out, kv, score, state_cache, norm_w,
            meta_dev, block_table_ptr,
            total_group_slots, group_slots_per_batch, num_batch,
            CACHE_SIZE=cache_size, HEAD_DIM=head_dim, RATIO=ratio,
            RATIO_PAD=padded_ratio, CHUNK_ROWS=min(padded_ratio, MAX_CHUNK_ROWS),
            eps=1e-20,
    ...
    if total_write_slots > 0:
        ...
        _cache_update_kernel[cache_grid](
            kv, score, state_cache,
            meta_dev, block_table_ptr,
            total_write_slots, write_slots_per_batch, num_batch,
            CACHE_SIZE=cache_size, HEAD_DIM=head_dim,
        )
...
def compressor_ref(x, wkv, wgate, state_cache, cmp_ratio, norm_w,
                   state_block_table=None, cu_seqlens=None, seqused=None,
                   start_pos=None):
    head_dim = wkv.shape[0]
    ratio = int(cmp_ratio)
    ...
    start_pos_list = _ints(start_pos) if start_pos is not None else [0] * num_batch
    block_table = (state_block_table.cpu().tolist() if state_block_table is not None
                   else list(range(num_batch)))
    cache_size = state_cache.shape[1]
    ...
    for b in range(num_batch):
        ...
        seg_rows = torch.stack([torch.cat([k, s]) for k, s in zip(kv, scores)]) if len(kv) else \
            torch.empty(0, 2 * head_dim, dtype=torch.float32, device=x.device)
        tokens = torch.cat([torch.stack(residual_rows) if residual_rows else
                            torch.empty(0, 2 * head_dim, dtype=torch.float32, device=x.device),
                            seg_rows])
    ...
    if cu_seqlens is None:
        ...
        out_full = torch.zeros(num_batch, rows_per_batch, head_dim,
                               dtype=torch.bfloat16, device=x.device)
        for b in range(num_batch):
            out_full[b, :group_counts_all[b]] = batch_outputs[b]
        return out_full
    out_offset = 0
    out_full = torch.zeros(min(x.shape[0], x.shape[0] // ratio + num_batch), head_dim,
                           dtype=torch.bfloat16, device=x.device)
    for b in range(num_batch):
