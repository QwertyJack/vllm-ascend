...
class DeepseekV41EagerAttentionImpl:
    ...
    def _attention(self, attn, q, positions, metadata, compressed_indices):
        ...
        # arch22 cannot safely merge sparse rows padded with -1. Keep the
        # masked reference path for these rows and prefill until native SMLA
        # has a validated valid-length contract. This containment is eager.
        is_prefill = (
            getattr(metadata.swa, "num_prefills", 0) > 0
            or getattr(metadata, "num_prefills", 0) > 0
        )
        has_padding = compressed_indices is not None and bool((compressed_indices < 0).any().item())
        if self.role.compress_ratio in (0, 1, 2) and not is_prefill and not has_padding:
    ...
    def forward(self, attn, positions, hidden_states):
        ...
        return attn.project_output(output, projected)
