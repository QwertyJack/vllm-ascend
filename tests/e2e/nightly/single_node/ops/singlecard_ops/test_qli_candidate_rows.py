...
def test_candidate_consumer_preserves_each_query_mask(ratio, query_count):
    ...
    key = torch.randint(-90, 90, (kv_len // block_size, block_size, 1, width),
    ...
    common = dict(cu_seqlens_q=qsl, seqused_k=lengths, cmp_residual_k=residual,
                  max_seqlen_q=query_count, layout_q="TND", layout_k="PA_BBND",
                  mask_mode=3, cmp_ratio=ratio)
    ...
    selected, _, _ = torch.ops._C_ascend.npu_quant_lightning_indexer_v2(
        query, key, weights, query_scale, key_scale, topk, 2,
        block_table=table, metadata=metadata, candidate_topk_index=candidates,
        candidate_mode=2, candidate_topk_blocks=512, candidate_block_size=8, **common)
