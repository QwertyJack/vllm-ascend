...
class AuroraTilingKeysTest(unittest.TestCase):
    ...
    def test_compressor_retains_model_dispatch_and_key_encoding(self):
        ...
        for arch in (220, 310):
            ...
            template_ids = (1, 2) if arch == 220 else (0, 1)
            rope_dtype = (1,) if arch == 220 else ()
            expected = {
                (1, 0, coff, 2, 1, template_id, *rope_dtype)
                for coff, template_id in itertools.product((1, 2), template_ids)
            }
            with self.subTest(arch=arch):
                ...
                self.assertEqual(
                    ...
                    declaration + ([["ROPE_DTYPE", 1, [0, 1]]] if arch == 220 else []),
                )

        for filename, count in (("models/deepseek_v4/compressor.py", 1), ("attention/context_parallel/dsa_cp.py", 2)):
            ...
            for call in calls:
                modes = {
                    ...
                    if kw.arg in ("rotary_mode", "cache_mode")
                }
                self.assertEqual(modes, {"rotary_mode": 2, "cache_mode": 1})
    ...
    def test_compressor_dtype_registration_matches_selected_templates(self):
        ...
        base, a2a3 = source.split("OpAICoreConfig config910;")
        for arch, config in (("arch32", a2a3), ("arch35", base)):
            with self.subTest(arch=arch):
                rows = []
                ...
                expected = {
                    ...
                    "ape": "ge::DT_FLOAT",
                    "norm_weight": "ge::DT_BF16" if arch == "arch32" else "ge::DT_FLOAT",
                ...
                self.assertRegex(host, r"\{X_NAME,\s*\{COMPRESSOR_DIM_NUM_2\}\}")
                for mode, value in (("ROTARY_MODE", 2), ("CACHE_MODE", 1)):
                    definitions = [
                        line for line in host.splitlines() if line.startswith(f"const std::vector<int> {mode} ")
                    ]
                    self.assertEqual(len(definitions), 1)
                    self.assertRegex(definitions[0], rf"\{{\s*{value}\s*\}}")
    ...
    def test_compressor_empty_input_does_not_instantiate_computation(self):
        ...
        for arch in (220, 310):
            with tempfile.TemporaryDirectory() as directory:
                ...
                variant = "perf" if arch == 220 else "full_load"
                for name in ("compressor_kernel.h", f"compressor_kernel_{variant}.h"):
    ...
    def test_model_call_sites_use_the_compiled_layouts(self):
        ...
        # the compiled template contract, not just this test's expected matrix.
        source = ast.parse((REPO_ROOT / "vllm_ascend/attention/dsa_v41.py").read_text())
        native = next(
            node for node in ast.walk(source) if isinstance(node, ast.FunctionDef) and node.name == "_native_attention"
        )
        calls = [
            node
        ...
        for call in calls:
            layouts = {
                kw.arg: ast.literal_eval(kw.value) for kw in call.keywords if kw.arg in ("layout_q", "layout_kv")
            }
            self.assertEqual(layouts, {"layout_q": "TND", "layout_kv": "PA_BBND"})
        ...
        self.assertEqual(layouts, {"layout_q": "TND", "layout_k": "PA_BBND"})
